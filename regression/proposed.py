...
from .jamshidi import LassoFeatureSelector, L2SFeatureSelector, FeatureSelector
...
class TransferLearning(ABC):
  def __init__(self, workload, system, base_data_path, target_data_path):
...
class ModelShift(TransferLearning):
  def __init__(self, workload, system, base_data_path: str, target_data_path: str, feature_selector: FeatureSelector = L2SFeatureSelector()):
...
class L2S(TransferLearning):
  ...
  def fit(self) -> None:
    X = self.finetune_data[self.parameters]
...
class DataReuse(TransferLearning):
  def __init__(self, workload, system, base_data_path: str, target_data_path: str, feature_selector: FeatureSelector = L2SFeatureSelector(), no_sample=False):
    self.feature_selector: FeatureSelector = feature_selector
